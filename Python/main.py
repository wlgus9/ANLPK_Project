...
def get_new_word_information(article_df):
    ...
    # 고유명사 결측치 처리
    train_processed_week['proper_nouns'] = train_processed_week['proper_nouns'].fillna('')
...
def get_new_words():
    ...
    ####### 일단 값 받아오는지 테스트용 - 아예 url 넣어둠
    articles = pd.read_csv('extract0307_0311/Article_preprocessed/preprocessed_IT_V5.csv')
    article_one = articles[articles['url'] == 'https://news.v.daum.net/v/EmOBz3D1Mo'].reset_index(drop=True)
